      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {projects.map((project, idx) => (
          <Link to={`/project/${project.id}`} key={idx} className="group cursor-pointer block">
            <div className="bg-zinc-900/80 rounded-2xl overflow-hidden mb-4 aspect-video relative shadow-lg border border-zinc-800 group-hover:border-cyan-400/50 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:shadow-cyan-500/20 transition-all duration-500">
              <img 
                src={project.thumbnail} 
                alt={project.title} 
                className="w-full h-full object-contain object-top group-hover:scale-105 transition-transform duration-700 ease-out opacity-90 group-hover:opacity-100 p-2"
                referrerPolicy="no-referrer"
              />
              
              {/* Color overlay transition */}
              <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/20 to-transparent opacity-0 group-hover:opacity-60 transition-opacity duration-500"></div>
            ...
            <div className="transform transition-transform duration-300 group-hover:translate-x-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-zinc-400 group-hover:text-cyan-400 font-mono transition-colors opacity-0 group-hover:opacity-100 translate-x-[-10px] group-hover:translate-x-0 duration-300">
                  Learn more -&gt;
                </span>
              </div>
            </div>
