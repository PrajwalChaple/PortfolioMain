...
export default function Chatbot() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error('GEMINI_API_KEY is not set. Chatbot will not work.');
        return;
      }
      console.log('Initializing Gemini with key:', apiKey.substring(0, 8) + '...');
      const ai = new GoogleGenAI({ apiKey });
      chatRef.current = ai.chats.create({
        model: 'gemini-2.5-flash',
        config: {
          systemInstruction: "You are an AI assistant for Prajwal Chaple's portfolio website. Prajwal is a B.Tech IT student and Full-Stack Web Developer skilled in React, TypeScript, Node.js, and Firebase. He built LearnGrid (an educational platform) and HireFilter (an AI hiring platform). Be helpful, concise, and tech-savvy. Format your responses in Markdown.",
        }
      });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: `Sorry, I encountered an error: ${error?.message || 'Unknown error'}` }]);
    } finally {
      setIsLoading(false);
    }
  };
